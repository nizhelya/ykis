...
$_db = $this->__construct();	
	...
	{
	case "otoplenie12.dbf":
				$_sql_trancate ='TRUNCATE TABLE YISGRAND.DBF_OTOPLENIE'; 
				$_result_trancate = $_db->query($_sql_trancate) or die('Connect Error (' . $_db->connect_errno . ') ' . $_db->connect_error);
				$table="YISGRAND.DBF_OTOPLENIE";
	break;
	case "otoplenie01.dbf":
				$table="YISGRAND.DBF_OTOPLENIE";
	break;
	case "GEK4.DBF":
...
$q = 'INSERT INTO '.$table.' (`address_id`,`sdate`,`fdate`, `kvartplata`, `otoplenie`,`podogrev`,`voda`, `tbo`, `stoki`,`kv`, `ot`,`gv`,`xv`,`tb`,`st`,`summa` )  values (';
foreach ($row as $key => $val){
if ($key == 'RASH' OR $key == 'DAT1'  OR  $key == 'DAT2'  OR   $key == 'SM1' OR  $key == 'SM2' OR $key == 'SM3' OR $key == 'SM4' OR $key == 'SM7' OR $key == 'SM8' OR $key == 'OB1' OR $key == 'OB2' OR $key == 'OB3' OR $key == 'OB4' OR $key == 'OB7' OR $key == 'OB8'  OR $key == 'SUMMA' ){ 
