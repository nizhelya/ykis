...
class QueryUpload
{
	...
	function import_dbf($dbf_file)
	{
		...
		$q = 'INSERT INTO  YISGRAND.LGOTNIK_UTSZN(`cart`, `inn`,`pasp`,`fio`, `cat`,`lg`,`st`,`fin`,`address_id` )  values (';
	foreach ($row as $key => $val){
	if ($key == 'NCARD' OR $key == 'IDPIL'  OR $key == 'PASPPIL' OR $key == 'FIOPIL' OR $key == 'KAT' OR $key == 'LGCODE' OR $key == 'DATEIN' OR $key == 'DATEOUT' OR $key == 'RAH')
	{
	...
	$q .= ')';
		...
		$_sql_call='CALL YISGRAND.update_lgota_from_utszn1(@success,@msg)';
