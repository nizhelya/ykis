...
class PrivatBank
{
	...
	function __construct( $MCH_ID, $MKEY, $SKEY ) {
		...
		$this->_LT     = 24;
		$this->_VER    = '3.00';
		$this->_SRVURL = 'http://localhost/ykis/pb/pb.php';
		//return $this->_SRVURL;
	}
	...
	function set_transaction($MCH_ID, $AMOUNT, $DESC, $TM ,$INFO='[]', $NOTE=NULL, $TYPE='11', $SRV_ID=284, $FEE=NULL, $CUR='UAH') {
		...
		$this->_TRNS[] = $TRN;
		//return prv($this->_TRNS);
	}
	
	function set_mode( $MODE ) {		
		switch( $MODE ) {
			case 'test': $this->_SRVURL = 'http://localhost/ykis/pb/test.php'; break;
			case 'real': $this->_SRVURL = 'http://localhost/ykis/pb/pb.php'; break;
		}
				//return $this->_SRVURL;
	...
	function create_payment() {		
		...
		$xmldata = PrivatBank::toXml( $this->_REQARR );		
	...
	public static function toXml($data, $rootNodeName = 'payment', $xml=null)
	{
		...
		foreach($data as $key => $value)
		{
			...
			if (is_array($value))
			{
				...
				PrivatBank::toXml($value, $rootNodeName, $node);
			}
			else 
			{
                $value = trim($value);
