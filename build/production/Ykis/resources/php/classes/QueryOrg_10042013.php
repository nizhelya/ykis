...
// GET RESULT
			...
			case "TekNachFilial":			  
			   $this->sql='SELECT filial_id, data, fdate, usluga, period, sum(zadol) as zadol, hzadol,edizm, qty, tarif, sum(nachisleno) as nachisleno, sum(perer) as perer,  sum(itogo) as itogo, sum(oplacheno) as oplacheno,  sum(dolg) as dolg,  hdolg FROM('
				  ...
				  .'CASE WHEN xkub+gkub > 0 THEN "куб" ELSE "чел" END as edizm,CASE WHEN xkub+gkub > 0 THEN xkub+gkub ELSE people END as qty,tarif,nachisleno-perer as nachisleno,perer,'
				  ...
				  .'CASE WHEN kub > 0 THEN "куб" ELSE "чел" END as edizm,CASE WHEN kub > 0 THEN kub ELSE people END as qty,tarif,nachisleno-perer as nachisleno,perer,'
...
// GET RESULT
...

	{
				...
				if(isset($params->toc) && ($params->toc)) {
				$this->toc = $params->toc;
			      } else {
				$this->toc = 0;
			      }
			      if(isset($params->day_gv) && ($params->day_gv)) {
				$this->day_gv = $params->day_gv;
			      } else {
				$this->day_gv = 0;
			      }
				...
				$this->nrx_gvs_d = 0;
			      ...
			      if(isset($params->xkub) && ($params->xkub)) {
				$this->xkub = $params->xkub;
			      } else {
				$this->xkub = 0;
			      }
			      if(isset($params->kub) && ($params->kub)) {
				$this->kub = $params->kub;
			      } else {
				$this->kub = 0;
			      }
			      if(isset($params->gkub) && ($params->gkub)) {
				$this->gkub = $params->gkub;
			      } else {
				$this->gkub = 0;
			      }
			      if(isset($params->norma) && ($params->norma)) {
				$this->norma = $params->norma;
			      } else {
				$this->norma = 0;
			      }
			      if(isset($params->podogrev) && ($params->podogrev)) {
				$this->podogrev = $params->podogrev;
			      } else {
				$this->podogrev = 0;
			      }
				...
				$this->gkal = 0;
			      ...
			      if(isset($params->gkal_perer) && ($params->gkal_perer)) {
				$this->gkal_perer = $params->gkal_perer;
			      } else {
				$this->gkal_perer = 0;
			      }
			      if(isset($params->xvoda) && ($params->xvoda)) {
				$this->xvoda = $params->xvoda;
			      } else {
				$this->xvoda = 0;
			      }
			      if(isset($params->gvoda) && ($params->gvoda)) {
				$this->gvoda = $params->gvoda;
			      } else {
				$this->gvoda = 0;
			      }
				...
				$this->nds = 0;
			      ...
			      if(isset($params->perer) && ($params->perer)) {
				$this->perer = $params->perer;
			      } else {
				$this->perer = 0;
			      }
			      if(isset($params->pnds) && ($params->pnds)) {
				$this->pnds = $params->pnds;
			      } else {
				$this->pnds = 0;
			      }
			      if(isset($params->nachisleno) && ($params->nachisleno)) {
				$this->nachisleno = $params->nachisleno;
			      } else {
				$this->nachisleno = 0;
			      }
			      if(isset($params->oplacheno) && ($params->oplacheno)) {
				$this->oplacheno = $params->oplacheno;
			      } else {
				$this->oplacheno = 0;
			      }
			      if(isset($params->dolg) && ($params->dolg)) {
				$this->dolg = $params->dolg;
			      } else {
				$this->dolg = 0;
			      }
				...
				$this->bank_id = (int) $params->bank_id;
			      } else {
				$this->bank_id = 0;
			      }

			      if(isset($params->rs) && ($params->rs)) {
				$this->rs = $_db->real_escape_string($params->rs);
			      } else {
				$this->rs = '';
			      }
				...
				$this->dteplomer_id = 0;
			      ...
			      if(isset($params->kalendar_hour) && ($params->kalendar_hour)) {
				$this->kalendar_hour = (int) $params->kalendar_hour;
			      } else {
				$this->kalendar_hour = 0;
			      }
			      if(isset($params->grafik_hour) && ($params->grafik_hour)) {
				$this->grafik_hour = (int) $params->grafik_hour;
			      } else {
				$this->grafik_hour = 0;
			      }
			      if(isset($params->work_day) && ($params->work_day)) {
				$this->work_day = (int) $params->work_day;
			      } else {
				$this->work_day = 0;
			      }
			...
			case "OrgVodaIns":
				...
				.$this->people.',YISGRAND.VODA.xkub='
				.$this->xkub.',YISGRAND.VODA.gkub='
				.$this->gkub.',YISGRAND.VODA.tarif='
				.$this->tarif.',YISGRAND.VODA.norma='
				.$this->norma.',YISGRAND.VODA.xvoda='
			...
			case "OrgStokiIns":
				...
				.$this->people.',YISGRAND.STOKI.xkub='
				.$this->xkub.',YISGRAND.STOKI.gkub='
				.$this->gkub.',YISGRAND.STOKI.tarif='
				.$this->tarif.',YISGRAND.STOKI.norma='
				.$this->norma.',YISGRAND.STOKI.xvoda='
			...
			case "OrgPodogrevIns":
				...
				.$this->kub.',YISGRAND.PODOGREV.toc='
	...
	public function Filial(stdClass $params)
	{
		...
		switch ($this->what) {
			...
			$this->sql='CALL YISGRAND.add_filial('.$this->golovnoe.','.$this->org_id.','.$this->raion_id.','.$this->street_id.','.$this->house_id.','.$this->address_id.','
						...
						.$this->nrx_xv_d.','.$this->pstoki.','.$this->qty_hour.',@filial_id,@success,@msg)';
						//print($this->sql);
			break;
			...
			$this->sql='CALL YISGRAND.update_filial('.$this->filial_id.','.$this->golovnoe.','.$this->org_id.','.$this->raion_id.','.$this->street_id.','.$this->house_id.','.$this->address_id.','
						...
						.$this->vkl_gvoda.','.$this->gvodomer.','.$this->xvodomer.','.$this->teplomer.','.$this->nrx_gvs_d.','
						.$this->nrx_xv_d.','.$this->pstoki.','.$this->qty_hour.',@success,@msg)';
						//print($this->sql);
			break;
			case "nachGvsFilial":
